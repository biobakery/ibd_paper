...
list(species = phylo_species,
  ...
  purrr::iwalk(function(phylo, feature_level) {
    ...
    p <- tb_uniqPres %>% 
      ...
      rotate_xaxis(90) 
    ggsave(filename = paste0(dir.output, "fig_", feature_level, "UniquePresent.pdf"),
           plot = p,
           width = 40,
           height = 10)
    p <- tb_uniqPres %>% 
    ...
    p %>% print() %>% 
      ggsave(filename = paste0(dir.output, "fig_", feature_level, "UniquePresentZoom.pdf"),
         plot = .,
         width = 15,
         height = 10)
    ...
    p <- tb_uniqAbs %>% 
      ...
      rotate_xaxis(90) + coord_flip() 
    p %>% print() %>% 
      ggsave(filename = paste0(dir.output, "fig_", feature_level, "UniqueAbsent.pdf"),
             plot = .,
             width = 15,
             height = 10)
  })
```
## Dataset-specific OTUS
```{r dataset-specific OTUS}
...
cowplot::plot_grid(p1, p2, ncol = 1) %>% 
  ...
  ggsave(filename = paste0(dir.output, "fig_libSizeCutoff.pdf"),
       width = 10,
       height = 8)
