---
title: "2-QC"
author: "Siyuan Ma"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: 
  html_document:
    df_print: paged
---
# Overview
- Filter samples based on read depth
- Filter features based on prevalence and abundance

```{r setup, echo=FALSE}
knitr::opts_knit$set(root.dir = normalizePath(".."))
```
```{r setup2, echo=FALSE}
rm(list = ls())
for(i.function in list.files("functions/", full.names = TRUE)) {
  source(i.function)
}
setup(getwd())
dir_output <- "results/2-QC/"
dir.create(dir_output)
```

# Load studies

```{r load datasets}
load("data/phyloseq/otus_prefilter.RData")
load("data/phyloseq/species_prefilter.RData")
load("data/phyloseq/genera_prefilter.RData")
```

# Filter samples

```{r filter samples and taxa}
print(c(nsamples(phylo_species_prefilter),
        dplyr::n_distinct(sample_data2(phylo_species_prefilter)$subject_accession),
        nsamples(phylo_genera_prefilter),
        dplyr::n_distinct(sample_data2(phylo_genera_prefilter)$subject_accession)))
phylo_species <- phylo_species_prefilter %>% 
  phyloseq::subset_samples(read_depth > 3000)
phylo_genera <- phylo_genera_prefilter %>% 
  phyloseq::subset_samples(read_depth > 3000)
print(c(nsamples(phylo_species),
        dplyr::n_distinct(sample_data2(phylo_species)$subject_accession),
        nsamples(phylo_genera),
        dplyr::n_distinct(sample_data2(phylo_genera)$subject_accession)))
```

# Filter taxa

```{r filter taxa}
cutoff_abundance <- c(1e-5, 1e-4, 1e-3)
cutoff_prevalence <- c(0.05, 0.1)
tb_param <-  tidyr::crossing(cutoff_abundance, cutoff_prevalence)
tb_filter <- (1:nrow(tb_param)) %>% 
  purrr::map_dfr(function(i_param) {
    cutoff_abundance <- tb_param$cutoff_abundance[i_param]
    cutoff_prevalence <- tb_param$cutoff_prevalence[i_param]
    list(phylo_genera,
         phylo_species) %>% 
      purrr::map2_dfr(c("genera", "species"), 
                      function(phylo, feature_level) {
                        phylo_rel <- phylo %>% 
                          to_relativeAbundance()
                        tb_presence <- phylo_rel %>% 
                          phyloseq_to_tb() %>% 
                          dplyr::group_by(feature, dataset_name) %>% 
                          dplyr::summarise(
                            n = n(),
                            present = sum(abundance > cutoff_abundance) > 
                              n*cutoff_prevalence
                          ) %>% 
                          dplyr::ungroup()
                        tb_presence %>% 
                          dplyr::group_by(feature) %>% 
                          dplyr::summarise(n_study = sum(present)) %>% 
                          dplyr::mutate(cutoff_abundance = cutoff_abundance,
                                        cutoff_prevalence = cutoff_prevalence,
                                        feature_level = feature_level)
                      })
  })
# tb_filter %>%
#   readr::write_tsv(path = paste0(dir_output, "filter.tsv"))
tb_filter_summary <- tb_filter %>%
  dplyr::group_by(feature_level, cutoff_abundance, cutoff_prevalence) %>%
  dplyr::summarise(nfeature = n(),
                   nfeature_nonUnique = sum(n_study > 1),
                   nfeature_unique = sum(n_study == 1)) %>%
  dplyr::arrange(feature_level,
                 dplyr::desc(cutoff_abundance), dplyr::desc(cutoff_prevalence)) %>%
  readr::write_tsv(path = paste0(dir_output, "filter_summarise.tsv")) 

# What if filtering is based on number of samples instead of percentages?
cutoff_abundance <- c(1e-5, 1e-4, 1e-3)
cutoff_n <- c(2, 5, 10)
tb_param_n <-  tidyr::crossing(cutoff_abundance, cutoff_n)
tb_filter <- (1:nrow(tb_param_n)) %>% 
  purrr::map_dfr(function(i_param) {
    cutoff_abundance <- tb_param_n$cutoff_abundance[i_param]
    cutoff_n <- tb_param_n$cutoff_n[i_param]
    list(phylo_genera,
         phylo_species) %>% 
      purrr::map2_dfr(c("genera", "species"), 
                      function(phylo, feature_level) {
                        phylo_rel <- phylo %>% 
                          to_relativeAbundance()
                        tb_presence <- phylo_rel %>% 
                          phyloseq_to_tb() %>% 
                          dplyr::group_by(feature, dataset_name) %>% 
                          dplyr::summarise(
                            n = n(),
                            present = sum(abundance > cutoff_abundance) >= 
                              cutoff_n
                          ) %>% 
                          dplyr::ungroup()
                        tb_presence %>% 
                          dplyr::group_by(feature) %>% 
                          dplyr::summarise(n_study = sum(present)) %>% 
                          dplyr::mutate(cutoff_abundance = cutoff_abundance,
                                        cutoff_n = cutoff_n,
                                        feature_level = feature_level)
                      })
  })
# tb_filter %>%
#   readr::write_tsv(path = paste0(dir_output, "filter.tsv"))
tb_filter_summary_n <- tb_filter %>%
  dplyr::group_by(feature_level, cutoff_abundance, cutoff_prevalence) %>%
  dplyr::summarise(nfeature = n(),
                   nfeature_nonUnique = sum(n_study > 1),
                   nfeature_unique = sum(n_study == 1)) %>%
  dplyr::arrange(feature_level,
                 dplyr::desc(cutoff_abundance), dplyr::desc(cutoff_prevalence)) %>%
  readr::write_tsv(path = paste0(dir_output, "filter_summarise_n.tsv")) 

phylo_genera <- 
  tb_filter %>% 
  dplyr::filter(feature_level == "genera",
                cutoff_abundance == 1e-4,
                cutoff_prevalence == 0.05,
                n_study >= 1) %>% 
  dplyr::pull(feature) %>% 
  phyloseq::prune_taxa(phylo_genera) %>% 
  prune_taxaSamples()
phylo_species <- 
  tb_filter %>% 
  dplyr::filter(feature_level == "species",
                cutoff_abundance == 1e-4,
                cutoff_prevalence == 0.05,
                n_study >= 1) %>% 
  dplyr::pull(feature) %>% 
  phyloseq::prune_taxa(phylo_species) %>% 
  prune_taxaSamples()
# sanity check
nsamples(phylo_genera)
nsamples(phylo_species)

save(phylo_genera, file = "data/phyloseq/genera.RData")
save(phylo_species, file = "data/phyloseq/species.RData")
```