...
# table(metadata_tmp$batch, metadata_tmp$disease)
...
for(variable in c("antibiotics",
                  "immunosuppressants",
                  "steroids",
                  "mesalamine_5ASA")) {
  metadata_tmp <- metadata_test
  variable_na <- paste0(variable, "_is_NA")
  metadata_tmp[, variable_na] <- is.na(metadata_tmp[, variable])
  metadata_tmp[, variable] <- fill_na(metadata_tmp[, variable])
  test <- vegan::adonis(dist_genera_adj ~ .,
                        data = metadata_tmp[, c("disease", variable_na, variable)],
                        permutations = n_permutations,
                        parallel = ncores)
  assign(paste0("fit_adonis_", variable),
         test)
...
fit_adonis_subtype <-
  fit_permanova_variable(D = dist_genera_adj,
                         ...
                         variable_class = "subject",
                         covariates = NULL,
                         block_covariates = "IBD",
                         block_variable = "subject_accession",
                         data = metadata_test,
                         permutations = n_permutations,
                         ncores = ncores)
save(fit_adonis_subtype, file = paste0(dir_output, "fit_adonis_subtype.RData"))
rm(fit_adonis_subtype)
...
tb1 <- readr::read_csv("tables/table1/assets/table1_metadata.csv")
for(study in studies) {
  ...
  for(variable in c("B.cat", "L.cat", "E.cat")) {
    ...
    if(any(is.na(metadata_test_tmp[, variable])) & !is.null(block_covariates)) {
      ...
      if(all(apply(table_test, 2, min) == 0)) {
        metadata_test_tmp[, variable] <- fill_na(metadata_test_tmp[, variable])
  ...
  for(variable in c("antibiotics",
    ...
    if(any(is.na(metadata_test_tmp[, variable])) & !is.null(block_covariates)) {
      ...
      if(all(apply(table_test, 2, min) == 0)) {
        ...
        variable_na <- paste0(variable, "_is_NA")
        metadata_test_tmp[, variable_na] <- is.na(metadata_test_tmp[, variable])
      }
    }
    if(study == "PROTECT") {
      test <- fit_permanova_variable(D = dist_genera_adj_tmp,
      ...
      metadata_test_tmp[, variable] <- fill_na(metadata_test_tmp[, variable])
      test <- vegan::adonis(dist_genera_adj_tmp ~ .,
                            data = metadata_test_tmp[, c(block_covariates,
                                                         variable_na,
                                                         variable)],
                            permutations = n_permutations,
                            parallel = ncores)
  ...
  if(length(unique(metadata_test_tmp$disease)) < 2) {
    fit_adonis_subtype <- NULL
  } else {
    if(length(unique(metadata_test_tmp$IBD)) < 2) {
      fit_adonis_subtype <- 
      ...
      if(length(unique(setdiff(metadata_test_tmp$disease, "control"))) == 2) {
        fit_adonis_subtype <- 
          fit_permanova_variable(D = dist_genera_adj_tmp,
                                 variable = "disease",
                                 variable_class = "subject",
  ...
  save(fit_adonis_subtype, file = paste0(dir_output, study, "/fit_adonis_sutype.RData"))
  rm(fit_adonis_subtype)
  ...
  if(tb1 %>% 
     dplyr::filter(study_full_name == study) %>% 
     extract2("Longitudinal sampling?") %>% 
     equals("Yes")) {
    fit_adonis_subject <-
      fit_permanova_variable(D = dist_genera_adj_tmp,
                             variable = "subject_accession",
