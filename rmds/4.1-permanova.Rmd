...
for(i_run in (1:nrow(tb_permanova))) {
  ...
  if(i_tb_permanova$study == "all") {
    ...
    i_metadata_test <- subset(metadata_test, dataset_name %in% i_tb_permanova$stud)
  }
  ...
  if(i_tb_permanova$phylo == "original") 
    i_dist <- subset_distance(dist_genera, rownames(i_metadata_test))
  if(i_tb_permanova$phylo == "adjusted") 
    i_dist <- subset_distance(dist_genera_adj, rownames(i_metadata_test))
  if(i_tb_permanova$phylo == "batch_adjusted") {
    i_dist <- subset_distance(dist_genera_adjBatch, rownames(i_metadata_test))
  }
