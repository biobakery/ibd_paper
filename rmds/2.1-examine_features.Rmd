...
# in the OTU table
p <- sample_data2(phylo_species_prefilter) %>% 
...
# samples passing 1000 or 3000 lib size
p1 <- sample_data2(phylo_species_prefilter) %>%
  ggplot(aes(x = dataset_name, fill = (original_read_count > 3000))) +
  geom_bar(stat = "count") +
  rotate_xaxis(30) +
  ggtitle("Cutoff = 3000")
p2 <- sample_data2(phylo_species_prefilter) %>%
  ggplot(aes(x = dataset_name, fill = (original_read_count > 1000))) +
  geom_bar(stat = "count") +
  rotate_xaxis(30) +
  ggtitle("Cutoff = 1000")
p <- cowplot::plot_grid(p1, p2, ncol = 1) %>%
  cowplot_title(title = "Lib size filtering cut off?")
print(p) %>%
...
tb_nTaxaSampleLibSize <- list(otus = phylo_otus_prefilter,
  ...
  purrr::imap_dfr(function(phylo, feature_level) {
    purrr::map_dfr(studies, function(study) {
      ...
      tibble::tibble(
        ...
        median_log10LibSize = sample_data2(phylo)$lib_size %>% 
          add(1) %>% log10 %>% 
          median()
      )
    })
