...
p_compareReadCounts <- 
  suppFig_readDepth(phylo_species_prefilter,
                      phylo_species_prefilter,
                      phylo_species_prefilter)$p_compareReadCounts
...
p1 <- sample_data2(phylo_species_prefilter) %>%
  ggplot(aes(x = dataset_name, fill = (read_depth > 3000))) +
  geom_bar(stat = "count") +
  rotate_xaxis(30) +
  ggtitle("Cutoff = 3000")
p2 <- sample_data2(phylo_species_prefilter) %>%
  ggplot(aes(x = dataset_name, fill = (read_depth > 1000))) +
  geom_bar(stat = "count") +
  rotate_xaxis(30) +
  ggtitle("Cutoff = 1000")
p <- cowplot::plot_grid(p1, p2, ncol = 1) %>%
  cowplot_title(title = "Read depth filtering cut off?")
print(p) %>%
...
tb_long <- studies %>% 
  purrr::map_dfr(
    function(study) {
      # This doesn't work!!
      # phylo_otus <- phylo_otus_prefilter %>%
      #   phyloseq::subset_samples(dataset_name == study)
      phylo_otus <- sample_data2(phylo_otus_prefilter) %>%
        dplyr::filter(dataset_name %in% study) %>% 
        magrittr::extract2("sample_accession_16S") %>% 
        phyloseq::prune_samples(phylo_otus_prefilter)
...
tb_nTaxaSampleReadDepth <- list(otus = phylo_otus_prefilter,
                              species = phylo_species_prefilter,
                              genera = phylo_genera_prefilter) %>% 
  purrr::imap_dfr(function(phylo, feature_level) {
    purrr::map_dfr(studies, function(study) {
      phylo <- sample_data2(phylo) %>%
        dplyr::filter(dataset_name %in% study) %>% 
        magrittr::extract2("sample_accession_16S") %>% 
        phyloseq::prune_samples(phylo)
      phylo <- phylo %>%
      ...
      tibble::tibble(
        ...
        median_log10ReadDepth = sample_data2(phylo)$read_depth %>% 
          add(1) %>% log10 %>% 
          median()
      )
    })
...
list(species = phylo_species_prefilter,
     genera = phylo_genera_prefilter) %>% 
...
```{r individual OTUS}
mat_tax_tmp <- tax_table2(phylo_species_prefilter)
mat_tax_tmp %>% 
...
# Examine feature prevalence/abundance distribution after filtering
These figures are fairly big so directly save to supplemental figures.
```{r feautre prevalence abundance}
source("functions/manuscript/suppFigures/suppFig_distPrevAbd.R")
tmp <- suppFig_meidumHighPrevalent(phylo_species)
tmp <- suppFig_featureMissingFromOne(phylo_species)
```