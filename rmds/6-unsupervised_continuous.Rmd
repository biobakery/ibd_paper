    ...
    p_validation <- tb_cor %>% 
      ...
      dplyr::mutate(batch_n = paste0(batch, " (", n, ")"),
                    batch_class = dplyr::case_when(
                      batch %in% c("PROTECT", "RISK", "CS-PRISM", "Pouchitis") ~ "Training",
                      batch == "nonIBD" ~ "nonIBD",
                      batch == "HC" ~ "HC",
                      TRUE ~ "Validation"
                    )) %>% 
...
```{r visualize scores}
tb_loading <- readr::read_tsv("results/6-unsupervised_continuous/genera/cor_cutoff_0.65/avg_loading.tsv")[, 1:3]
