...
- Add metadata columns (baseline indicator, library size, etc.)
```{r setup, echo=FALSE}
...
df_metadata <- sample_data2(phylo_species_prefilter)
```
...
#   nrow()
df_metadata$`lib_size` <- 
  apply(otu_table2(phylo_otus_prefilter), 2, sum)[df_metadata$sample_accession_16S]
# sanity check
ncol(df_metadata) == ncol(sample_data2(phylo_otus_prefilter)) + 4
```
...
# sanity check
ncol(df_metadata) == ncol(sample_data2(phylo_otus_prefilter)) + 5
all(df_metadata$sample_accession_16S %in% sample_names(phylo_otus_prefilter))
rownames(df_metadata) <- df_metadata$sample_accession_16S
```
```{r save these combined phyloseq objects prune features that are just not there}
...
save(phylo_otus_prefilter, file = "data/phyloseq/otus_prefilter.RData")
save(phylo_species_prefilter, file = "data/phyloseq/species_prefilter.RData")
save(phylo_genera_prefilter, file = "data/phyloseq/genera_prefilter.RData")
```
