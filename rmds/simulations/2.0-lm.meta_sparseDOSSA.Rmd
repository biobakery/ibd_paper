...
exposure <- function(batch, imbalance) { # helper funciton for simulating exposure metadata
  ...
  exposure[batch == 1] <- 1 * (sample.int(n = sum(batch == 1)) <= 
                                 round(sum(batch == 1) * (0.5 - imbalance), digits = 0)) 
  exposure[batch == 2] <- 1 * (sample.int(n = sum(batch == 2)) <= 
                                 round(sum(batch == 2) * (0.5 + imbalance), digits = 0))
  return(exposure)
}
tb_sim <- tb_simSetup %>% 
  dplyr::group_by(i) %>% 
  dplyr::mutate(df_metadata = list(
    data.frame(batch = (1 + (sample.int(n = nSample) <= round(nSample/2, digits = 0))) %>%
                 as.factor(),
