...
tb_results <- list(results, results_correct) %>% 
  purrr::map2_dfr(
    names(colors_simulation_continuous),
    ~ .x %>% 
      purrr::imap_dfr(
        ~ tibble::tibble(results = list(.x$fit.continuous),
                         i = .y)
      ) %>% 
      dplyr::mutate(Data = .y)
  ) 
...
tb_summary <- tb_results %>% 
  dplyr::mutate(Data = factor(Data, levels = names(colors_simulation_continuous)),
                Cor = results %>% 
...
tb_plot <- tb_summary %>% 
  ...
  dplyr::filter(!(effect_batch == 0 &
                  Data == "MMUPHin corrected, MMUPHin analyzed"))
plist <- (1:nlevels(tb_sim$effect_score)) %>% 
    purrr::map(function(i) {
      i.effect <- levels(tb_sim$effect_score)[i]
      p <- tb_plot %>% 
        dplyr::filter(effect_score == i.effect) %>% 
        ggplot(aes(x = effect_batch, y = abs(Cor), color = Data)) +
        geom_boxplot() +
        facet_grid(nMicrobe_percSpike ~ nSample_nBatch) +
        ...
        xlab("Batch effect") +
        ylab("Correlation") +
        ggtitle(paste0("Effect of Score  = ", 
                       i.effect %>%  
                         as.numeric))
      if(i >= 2)
        p <- p + theme(legend.position = "none")
