...
for(feature_level in c("genera", "species")) {
  ...
  for(study in studies_batch) {
    ...
    covariates_tmp <- c("disease") %>% 
      purrr::map2_chr(list(metadata[samples_tmp, ]$disease),
                      function(covariate, values) 
                        ifelse(dplyr::n_distinct(values, na.rm = TRUE) > 1,
                               covariate,
                               NA_character_)) %>% 
      setdiff(NA_character_)
    if(length(covariates_tmp) == 0) covariates_tmp <- NULL
    ...
    dist_after <- mat_otu_tmp %>% phyloseq::otu_table(taxa_are_rows = TRUE) %>%
      phyloseq::phyloseq(sample_data(metadata[samples_tmp, ])) %>%
      to_relativeAbundance() %>%
      phyloseq::distance(method = "bray")
    # PERMANOVA
    l_fit_adonis[[study]] <-
  ...
  distance_after_batch <- phylo_adj %>%
    to_relativeAbundance() %>%
    phyloseq::distance(method = "bray")
  assign(paste0("dist_", feature_level, "_after_batch"), distance_after_batch)
  save(list = paste0("dist_", feature_level, "_after_batch"),
       file = paste0("data/distance/", "dist_", feature_level, "_after_batch.RData"))
