...
for(study in studies) {
  ...
  l_fit_adonisMarg <- paste0(c("race",
    ...
    purrr::map(function(variable) {
      ...
      as.formula(paste0("dist_genera_adj_tmp~(", variable,
                        "==\"NA\")", "+", variable)) %>%
  ...
  l_fit_adonisCond <- paste0(c("B.cat", "L.cat", "E.cat",
    ...
    purrr::map(function(variable) {
      ...
      if(length(unique(metadata_test_tmp$disease)) < 2) {
        ...
        as.formula(paste0("dist_genera_adj_tmp~disease+(", variable,
                          "==\"NA\")", "+", variable)) %>%
          vegan::adonis(data = metadata_test_tmp, 
                        permutations = n_permutations) %>% 
          return()
  ...
  if(length(unique(metadata_test_tmp$disease)) < 2) {
    ...
    fit_adonis_disease <-
      vegan::adonis(dist_genera_adj_tmp ~ disease,
                    data = metadata_test_tmp, permutations = n_permutations)
  }
  save(fit_adonis_disease, 
       file = paste0(dir_outputChunk, "fit_adonis_disease.RData"))
  ...
  if(length(unique(metadata_test_tmp$body_site)) < 2) {
    ...
    if(all(metadata_test_tmp$sample_type == "biopsy")) {
      fit_adonis_bodySite <-
        vegan::adonis(dist_genera_adj_tmp ~ body_site,
                      data = metadata_test_tmp, permutations = n_permutations)
    } else {
      fit_adonis_bodySite <-
        vegan::adonis(dist_genera_adj_tmp ~ sample_type + body_site,
                      data = metadata_test_tmp, permutations = n_permutations)
    }
  }
  save(fit_adonis_bodySite, file = paste0(dir_outputChunk, "fit_adonis_bodySite.RData"))
  ...
  fit_adonis_subject <-
    vegan::adonis(dist_genera_adj_tmp ~ subject_accession,
                  data = metadata_test_tmp, permutations = n_permutations)
  save(fit_adonis_bodySite, file = paste0(dir_outputChunk, "fit_adonis_subject.RData"))
}

studies %>% 
  purrr::map_dfr(function(study) {
    load(paste0(dir_outputChunk, study, "/l_fit_adonisMarg.RData"))
    tb_marg <- c("race",
                 "gender",
                 "age.cat",
                 "age_at_diagnosis.cat") %>%
...
#          unique %>% length %>% divide_by(4),
#        width = 15)
# tax_table2(phylo_genera_adj) %>% 
#   as.data.frame() %>% 
#   dplyr::filter(Rank5 %in% "f__Enterococcaceae") %>% 
#   View
```
